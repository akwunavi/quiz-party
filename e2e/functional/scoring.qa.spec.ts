...
const ok = (questionIndex: number, extra: Partial<ScoredAnswer> = {}): ScoredAnswer => ({
  questionIndex,
  isCorrect: true,
  ...extra,
})
const wrong = (questionIndex: number, extra: Partial<ScoredAnswer> = {}): ScoredAnswer => ({
  questionIndex,
  isCorrect: false,
  ...extra,
})
...
describe('QA: scoring regression', () => {
  ...
  it('jeopardy: awards tile value only for correct tiles', () => {
    expect(scoreJeopardy([
      { value: 100, isCorrect: true },
      { value: 200, isCorrect: false },
      { value: 300, isCorrect: null },
      { value: 400, isCorrect: true },
    ])).toBe(500)
  })
  ...
  it('melody: uses stake when present and bid seconds as fallback', () => {
    ...
    expect(scoreMelody([
      ok(0, { stake: 2, bidSeconds: 5 }),
      ok(1, { stake: null, bidSeconds: 5 }),
      ok(2, { stake: null, bidSeconds: 8 }),
      wrong(3, { stake: 2, bidSeconds: 5 }),
    ])).toBe(5)
  })
