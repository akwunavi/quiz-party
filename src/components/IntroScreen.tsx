...
export function IntroScreen({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    // «Интро» — одноразовый ~20-секундный спектакль на ОБЩИЙ проектор для
    // всего зала, а не персональный элемент интерфейса, который человек
    // видит на каждом экране игры. На боевом прогоне это молча включило
    // урезанную ветку ниже (мелкий текст без анимации, музыка мельком) —
    // судя по всему, на машине с проектором где-то на уровне ОС/браузера
    // стоит «уменьшить движение», а повлиять на эту настройку посреди
    // игры ведущий не может. Раньше в артефакте-прототипе было то же
    // решение по той же причине: демка должна показывать анимацию, а не
    // тихо подчиняться чужому системному флагу. Урезанная ветка ниже не
    // удалена — просто сейчас недостижима; вернуть можно явным пропом,
    // если понадобится настоящий accessibility-режим по запросу ведущего.
