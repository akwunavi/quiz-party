...
export interface PackSettings {
  ...
  /** Заставка «Интро» — киберпанк-анимация перед первым раундом (отсчёт,
...
export interface InfoSlide {
  ...
  show_rounds?: boolean
  /** Показать блок статистики пакета: раунды, вопросы, мини-игра, музыка,
   *  примерное время игры. Числа считает lib/duration.ts:packStats. */
  show_stats?: boolean
...
export interface Pack {
   ...
   *  сам редактор про game_sessions ничего не знает. Полная история
   *  отыгрышей (не только последний) — в таблице pack_plays (0013),
   *  это поле оставлено как быстрый фолбэк для пакетов, доигранных до
   *  0013 (у них в pack_plays может не быть ничего, кроме бэкфилла). */
  last_game_id?: string | null
  created_at: string
  updated_at: string
}

/** Одна строка истории отыгрышей пакета (миграция 0013). */
...
export interface MelodyState {
  ...
  /** «Своя игра»: что сейчас с открытой плиткой. Живёт ЗДЕСЬ, а не в памяти
   *  вкладки проектора (как было до 8.86) — иначе пульт ведущего в телефоне
   *  не знает ни того, что плитка открыта, ни того, показан ли ответ, и
   *  управлять ей с телефона нечем. Отдельного поля в БД не заводим: `melody`
   *  — уже общий jsonb-мешок состояний механик (там же живут скачки),
   *  миграция для этого не нужна.
   *  tile — сквозной номер плитки в раунде, null/undefined — плитка закрыта;
   *  answer — нажато ли «Показать ответ»;
   *  replay — счётчик «переслушать»: любое изменение числа = ещё один запуск
   *  трека (сам факт «играет» состоянием не является — это событие). */
...
export interface Team {
  ...
  /** Момент регистрации (миграция 0001, колонка была в базе с самого
   *  начала — просто не была нужна в типе до сортировки лобби). Может не
   *  быть у совсем старых строк, если такие переживут ручную миграцию без
   *  дефолта — на этот случай сортировка по нему в лобби подстраховывается
   *  фолбэком на id. */
...
export interface Answer {
  ...
  updated_at: string
  // ставится один раз при вставке (см. миграцию 0009) — момент первой
  // отправки ответа, в отличие от updated_at, которая двигается и при
