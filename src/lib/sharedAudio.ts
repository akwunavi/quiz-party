...
export function playShared(src: string, startAt = 0): HTMLAudioElement {
  ...
  void playAudio(el, src, startAt, isStale).then(() => {
    // Если это воспроизведение больше не «текущее» — глушим немедленно, не
    // дожидаясь, пока оно само кому-то помешает (защитный дубль на случай
    // пути 1 — прямого play() без fetch, где isStale внутри playAudio не
    // проверяется, потому что скачивания не было).
    if (isStale()) { try { el.pause() } catch { /* уже мёртв */ } }
  })
  return el
}
