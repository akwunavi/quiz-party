...
describe('ничья: сравнение по раундам', () => {
  ...
  it('пример Ивана: 6-8-6 против 7-5-8 — выше вторая (сильнее финишировала)', () => {
    ...
    expect(r[0].team.id).toBe('b')     // 8 > 6 в ПОСЛЕДНЕМ раунде
    expect(r.map(x => x.place)).toEqual([1, 1])   // место общее
  })

  // Этот кейс различает старое и новое правило: по раундам-от-начала
  // выиграла бы A (впереди уже в первом раунде), по раундам-от-конца
  // (актуальное правило, подтверждено ведущим) выигрывает B — сильный
  // финиш перевешивает сильный старт.
  it('решает финиш, а не старт: впереди в 1-м раунде — не значит выше', () => {
    const teams = [t('a', 'Стартовая'), t('b', 'Финишная')]
    const totals = new Map([['a', 15], ['b', 15]])
    const rounds = new Map([['a', [10, 0, 5]], ['b', [0, 10, 5]]])
    const r = rankTeams(teams, totals, [], rounds)
    expect(r[0].team.id).toBe('b')     // 10 > 0 во 2-м (последнем различающемся) раунде
  })
