...
export async function loadPack(packId: string, force = false): Promise<LoadedPack> {
  if (!force && memCache.has(packId)) return memCache.get(packId)!
  ...
  try {
    ...
    const { data: questions, error: e3 } = roundIds.length
      ? await supabase.from('pack_questions').select('*')
          .in('round_id', roundIds).eq('hidden', false).order('position')
      : { data: [], error: null }
    if (e3) throw e3
    ...
    memCache.set(packId, loaded)
    try { localStorage.setItem(LS_KEY(packId), JSON.stringify(loaded)) } catch { /* full */ }
    return loaded
  } catch (err) {
