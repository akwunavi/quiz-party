...
/** Число, которое получит `jp.replay` СРАЗУ после открытия плитки —
 *  общая точка с оптимистичным рендером на проекторе (9.36).
 *
 *  Раньше проектор, кликнув по плитке, сразу рисовал модалку с номером
 *  плитки (`tileLocal`, чтобы не ждать пару секунд опроса), но `replay`
 *  для неё брал из ЕЩЁ СТАРОГО `gameState.melody` — значения ПРЕДЫДУЩЕЙ
 *  плитки. Как только опрос (раз в 2 сек) доносил настоящий, уже
 *  увеличенный `jp.replay`, число менялось, эффект в `TileModal` читал
 *  это как «нажали переслушать» и запускал трек заново с нуля — ровно
 *  через секунду-две после честного старта. Снаружи это выглядело как
 *  «трек играет секунду и начинается заново».
 *  Считать предсказанное число тем же способом, что и настоящий переход
 *  (`jpOpen`), — единственный способ, чтобы оптимистичный рендер и ответ
 *  опроса совпадали и не дёргали эффект. */
export function jpNextReplay(m: MelodyState | undefined): number {
  return (m?.jp?.replay ?? 0) + 1
}
...
export function jpOpen(m: MelodyState, tile: number): MelodyState {
  return { ...m, jp: { tile, answer: false, replay: jpNextReplay(m) } }
}
